...
/* Chassis box position: 0=none, 1/2/3=arrived at box # */
static volatile uint8_t chassis_box_position = 0U;
...
void HAL_FDCAN_RxFifo0Callback(FDCAN_HandleTypeDef *hfdcan, uint32_t notifications)
{
  ...
  while (HAL_FDCAN_GetRxFifoFillLevel(hfdcan, FDCAN_RX_FIFO0) > 0U) {
    if (HAL_FDCAN_GetRxMessage(hfdcan, FDCAN_RX_FIFO0, &header, data) != HAL_OK) continue;
    ...
    if (hfdcan->Instance == FDCAN1) {
      ...
      if (header.Identifier == 0x66U) {
        chassis_box_position = data[0];
      }
    }
  }
}
...
{
  return chassis_box_position;
}


void CanBus_ClearChassisBoxPosition(void)
{
  chassis_box_position = 0U;
}
